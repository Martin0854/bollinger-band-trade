"""
Phase 1 Backtest: Volume Filter + RSI Filter
...
def run_backtest_for_year(year: int, kospi_stocks: list) -> dict:
    """특정 연도의 백테스트 실행"""
    print(f"\n{'='*80}")
    print(f"📅 {year}년 백테스트 (Phase 1: Volume + RSI)")
    print(f"{'='*80}")

    # 연도 설정
    config = BacktestConfiguration.from_yaml("runs/configs/phases/phase1_volume_rsi.yaml")
...
print("=" * 80)
print("Phase 1 Backtest: Volume Filter + RSI Filter")
...
print(f"\n전략: Phase 1 (Volume + RSI)")
print(f"평균 수익률: {avg_return:.2f}%")
print(f"평균 승률: {avg_win_rate:.2f}%")

print("\n⚠️  Phase 1 한계점:")
print("   - MACD 필터 부재: 추세 확인 없이 진입하여 손실 위험")
print("   - 신뢰도 스코어링 부재: 신호 품질 평가 불가")
print("   - Phase 3 권장: MACD + Confidence 추가로 평균 +3.62%p 개선")
