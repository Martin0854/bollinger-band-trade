...
class WizardPosition:
    ...
    def from_dict(cls, data: dict) -> "WizardPosition":
        # Handle backward compatibility for old data without partial_take_profit_executed
        if "partial_take_profit_executed" not in data:
            data = {**data, "partial_take_profit_executed": False}
        return cls(**data)
...
class WizardPortfolioState:
    ...
    def from_dict(cls, data: dict) -> "WizardPortfolioState":
        return cls(
            ...
            positions=[WizardPosition.from_dict(p) for p in data.get("positions", [])],
            trade_history=[TradeRecord.from_dict(t) for t in data.get("trade_history", [])],
            pending_orders=[PendingOrder.from_dict(o) for o in data.get("pending_orders", [])],
                ...
                if order.action == "BUY":
                    ...
                    for j, pos in enumerate(state.positions):
                        if pos.stock_code == order.stock_code:
                            realized_pnl = (order.actual_price - pos.entry_price) * order.quantity
